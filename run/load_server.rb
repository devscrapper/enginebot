  ...
  def receive_data param
    ...
    case data["cmd"]
      ...
      when "Choosing_landing_pages"
        #TODO: selectionner (direct_medium_percent,organic_medium_percent, referral_medium_percent) de l'objectif du jour dans la base de données
        label = data["label"]
        date_building = data["date_building"]
...
ARGV.each { |arg|
  ...
  #TODO passer en parametre le nom de la base de données
} if ARGV.size > 0
...
EventMachine.run {
  ...
  #TODO connection to base de données
