...
class Flow
  ...
  # :policy : un type de policy : rank ou traffic
  ...
  def initialize(dir, type_flow, policy, label, date, vol=nil, ext=".txt")
    ...
    @ext = ext
    raise StandardError, "Flow not initialize" unless @dir && @type_flow && @policy && @label && @date && @ext
  end
  ...
  def move(to_path)
    raise StandardError, "target <#{to_path}> is not valid" unless File.exists?(to_path) && File.directory?(to_path)
    raise StandardError, "Flow <#{absolute_path}> not exist" unless exist?
    FileUtils.cp(absolute_path, to_path)
    ...
    if @vol.nil?
      ...
      if vol.nil?
        ...
        begin
          ...
          raise StandardError, "cannot push vol <#{@vol}> of flow <#{basename}> to input_flow server #{input_flows_server_ip}:#{input_flows_server_port} : #{e.message}"
        else
  ...
  def push_vol(authentification_server_port,
    ...
    begin
      ...
      raise StandardError, "cannot ask a new authentification to localhost:#{authentification_server_port} : #{e.message}"
    else
